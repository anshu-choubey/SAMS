...
class StudentRepository @Inject constructor(
    ...
    suspend fun getStoredFaceEmbedding(): Result<FaceVerificationData?> {
        return try {
            val response = apiService.getStoredFaceEmbedding()
            if (response.success && response.data != null) {
                Result.success(FaceVerificationData(
                    faceEmbedding = response.data["face_embedding"],
                    faceRegistered = response.data["face_registered"]?.toBooleanStrictOrNull() ?: false,
                    threshold = response.data["threshold"]?.toIntOrNull() ?: 85
                ))
            } else {
                Result.success(null)
            }
